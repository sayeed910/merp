...
@section('content')
    ...
    <div class="col-lg-3">
        <div class="form-group">
            <select name="customer form-control" id="customerSelect" style="width: 100%">
                <option></option>
    ...
    <script>
        $(document).ready(() => {
            ...
            $('#submit').on('click', () => {
                ...
                order.customerId = $('#customerSelect').val();

                console.log(order);
                $.ajax({'url': "{{url("/admin/sale-orders")}}", 'method': 'POST', data: order}).done(function(response){
                    $.notify("Sale Order Created Successfully", {position: 'top center', className:'success'});
                    setTimeout(function(){
                        window.location.href="{{url("/admin/sale-orders")}}";
                    }, 1000);
                    console.log(response);
                }).fail(function(){
                    $.notify("Could not save sale order", {position: 'top center', className:'error'});
                });
            });
